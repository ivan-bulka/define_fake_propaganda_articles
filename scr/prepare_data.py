...
# label IDs of the target class (label) setup as a dictionary
classes_map = {
    -1: 0,
    0: 1,
    1: 2
...
def _preprocess_file(file,
    ...
    print('df[input_ids] after calling convert_to_bert_input_ids: {}'.format(
        df['input_ids']))
    ...
    df = df[['article_id', 'class', 'label_id', 'input_ids', 'article_body']]
    df = df.reset_index(drop=True)
    ...
    # balance the dataset by class down to the minority class
    if balance_dataset:
        df_unbalanced_grouped_by = df.groupby('class')
    ...
    ## write TSV Files
    df_train.to_csv('{}/part-{}-{}.tsv'.format(train_data, args.current_host,
                                               filename_without_extension), sep='\t',
                    index=False)
    df_validation.to_csv(
        '{}/part-{}-{}.tsv'.format(validation_data, args.current_host,
                                   filename_without_extension), sep='\t', index=False)
    df_test.to_csv('{}/part-{}-{}.tsv'.format(test_data, args.current_host,
                                              filename_without_extension), sep='\t',
                   index=False)
