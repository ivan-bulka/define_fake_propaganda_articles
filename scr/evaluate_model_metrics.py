...
def encode_article(article_body, max_seq_length):
...
def predict_fn(input_data, model, max_seq_length):
    ...
    for jsonline in jsonlines:
        # features[0]:  article_body
        # features[1..n]:  is anything else (we can define the order ourselves)
        # Example:  
        #    {"features": ["This is good."]}        
        #
        article_body = json.loads(jsonline)["features"][0]

        input_ids, attention_mask = encode_article(article_body, max_seq_length)
...
def process(args):
    ...
    df_test_articles = pd.DataFrame(columns=['class', 'article_body'])
    df_test_articles.head()
    ...
    for file in input_files:
        ...
        df_temp = pd.read_csv(file_path,
                              sep='\t', 
                              usecols=['class', 'article_body'])
        df_test_articles = df_test_articles.append(df_temp)
        df_test_articles.head()

    df_test_articles = df_test_articles #.sample(n=2000)
    df_test_articles.shape
    df_test_articles.head()

    y_test = df_test_articles['article_body'].map(predict)
    y_actual = df_test_articles['class'].astype('int64')
